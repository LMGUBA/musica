...
def get_audio_url():
    """Obtener URL de audio directo para reproducción con múltiples intentos"""
    try:
        ...
        if not audio_url:
            ...
            audio_url = get_audio_url_fallback(video_url)
        
        # Método 3: Si aún falla, intentar con configuración más permisiva
        if not audio_url:
            print("Método 2 falló, intentando método permisivo...")
            audio_url = get_audio_url_permissive(video_url)
        ...
        if audio_url:
            ...
            return jsonify({
                "error": "No se pudo obtener URL de audio. El video podría estar restringido o no disponible.",
                "suggestion": "Intenta con otra canción"
            }), 503
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for fmt in formats:
                if (fmt.get('acodec') and fmt.get('acodec') != 'none' and 
                    ...
                    fmt.get('filesize_approx', 0) < 50000000):  # Límite de 50MB
